...
- 每一步都解释为什么。
- 把用户当代码小白，但不要太啰嗦。
- 默认不要修改仓库文件，代码交给用户自己写；AI 只给用户需要修改 / 新增的代码，并讲清楚。
- 给代码时不要直接贴一长段整个类，除非用户明确要求。
- 给代码时按“改哪里 -> 贴具体方法 -> 解释方法作用、参数、返回值、使用场景”的格式。
- 如果是改已有方法，说明“替换哪个方法”；如果是新增方法，说明“加在哪个方法后面或哪个类里”。
- 解释要写在代码外。
- 不要在代码里写注释，除非用户明确要求。
...
4. 验证方式
   - 编译命令
   - HTTP / gRPC / DB / Redis / 异常验收点
```

方法解释示例：

```text
isBanned(Long userId)
- 作用：判断业务用户 ID 是否封禁。
- 参数 userId：业务用户 ID，对应 user_info.user_id，不是数据库内部 id。
- 返回值：true 表示封禁，false 表示未封禁。
- 使用场景：gateway 登录发 token 前、post-service 发帖前、其他业务兜底校验。
```
...
2. 先确认影响范围。
3. 先告诉用户准备改哪些文件。
4. 用户确认或明确授权后再改。默认只讲解和给代码，不直接改文件。
5. 不要改无关文件。
6. 不要覆盖用户未提交改动。
7. 不要把未实现功能写成已完成。
8. 改完必须编译或给出无法编译的原因。

用户明确说“你来改 / 交给你做 / 帮我改文件 / 帮我完成”时，才可以修改文件。
如果用户只是说“开始这一步 / 继续 / 讲一下怎么做”，默认理解为用户要自己写代码，AI 只给方法和解释。
